...
MAX_ALLOWED_DOMAINS_JSON_CHARS = 8_194
MAX_SOURCE_URLS_JSON_CHARS = 6_200
MAX_PROCESSED_BYTES = 48_000
MAX_EVIDENCE_CHARS = 12_000
MAX_EVIDENCE_UTF8_BYTES = 48_000
MAX_PROMPT_BYTES = 96_000
AUDIT_PROMPT_HEADROOM_BYTES = 16_000
...
_TEXTUAL_APPLICATION_MEDIA_TYPES = (
    "application/json",
    "application/ld+json",
)
...
def _request_digest(claim: str, sources_json: str) -> str:
    payload = (
        str(len(POLICY_VERSION))
        + ":"
        + POLICY_VERSION
        + str(len(claim))
        + ":"
        + claim
        + str(len(sources_json))
        + ":"
        + sources_json
    )
...
def _parse_string_array(
    ...
    minimum: int,
    maximum: int,
    maximum_raw_chars: int,
) -> list[str]:
    if not isinstance(value, str) or len(value) > maximum_raw_chars:
        raise gl.vm.UserError(
            f"{ERROR_EXPECTED} {label} exceeds the encoded input limit"
        )
    try:
    ...
    if len(parsed) < minimum or len(parsed) > maximum:
        raise gl.vm.UserError(
            f"{ERROR_EXPECTED} {label} must contain between {minimum} and {maximum} items"
        )
...
def _canonical_domain(value: str) -> str:
    if len(value) > 253:
        raise gl.vm.UserError(
            f"{ERROR_EXPECTED} Allowed domains must be plain public hostnames"
        )
    ...
    if (
        ...
        or ":" in domain
        or "@" in domain
        or not _hostname_is_valid(domain)
    ):
        raise gl.vm.UserError(
            f"{ERROR_EXPECTED} Allowed domains must be plain public hostnames"
        )
...
def _canonical_allowed_domains(value: str) -> str:
    raw_domains = _parse_string_array(
        value,
        "allowed_domains",
        1,
        MAX_ALLOWED_DOMAINS,
        MAX_ALLOWED_DOMAINS_JSON_CHARS,
    )
    domains: list[str] = []
...
def _canonical_source_url(value: str, allowed_domains: list[str]) -> str:
    if len(value) > MAX_URL_CHARS:
        raise gl.vm.UserError(
            f"{ERROR_EXPECTED} Source URL length is outside the allowed range"
        )
    url = value.strip()
    ...
    for character in url:
        ...
        if (
            ...
            raise gl.vm.UserError(
                f"{ERROR_EXPECTED} Source URLs cannot contain whitespace or control characters"
            )
    ...
    if len(authority) == 0 or "@" in authority or ":" in authority:
        raise gl.vm.UserError(
            f"{ERROR_EXPECTED} Source URLs cannot contain credentials, IP literals, or explicit ports"
        )

    hostname = authority.lower()
    if not _hostname_is_valid(hostname):
        raise gl.vm.UserError(
            f"{ERROR_EXPECTED} Source URL must use a public hostname"
        )
    ...
    if len(allowed_domains) > 0:
        ...
        if not permitted:
            raise gl.vm.UserError(
                f"{ERROR_EXPECTED} Source hostname is not permitted by this deployment"
            )
...
def _canonical_sources(
    ...
    allowed_domains = _parse_string_array(
    ...
    raw_urls = _parse_string_array(
        source_urls_json,
        "source_urls",
        MIN_SOURCES,
        MAX_SOURCES,
        MAX_SOURCE_URLS_JSON_CHARS,
    )
...
class _VisibleTextExtractor(HTMLParser):
    ...
    def handle_startendtag(self, tag: str, attrs) -> None:
        ...
        if self.html_mode and tag_name in _HTML_NONVOID_HIDDEN_ELEMENTS:
            # HTML ignores self-closing syntax on non-void hidden elements such
            # as <script/>. Suppress content until a matching end tag appears.
            self.hidden_tags.append(tag_name)

    def handle_endtag(self, tag: str) -> None:
        tag_name = tag.lower()
        if (
            len(self.hidden_tags) > 0
            and tag_name == self.hidden_tags[-1]
        ):
...
def _media_type_is_textual(media_type: str) -> bool:
    ...
    if media_type in _MARKUP_MEDIA_TYPES:
        return True
    return media_type.startswith("application/") and (
...
def _content_range_proves_complete(value: str, body_length: int) -> bool:
    ...
    if (
        not _is_bounded_ascii_decimal(start_text)
        or not _is_bounded_ascii_decimal(end_text)
        or not _is_bounded_ascii_decimal(total_text)
    ):
        return False
    ...
    return (
        ...
        and total > 0
        and end == total - 1
        and body_length == total
        and total <= MAX_PROCESSED_BYTES
    )
...
def _content_length_matches(value: str, body_length: int) -> bool:
    if len(value) == 0:
        return True
    return (
        _is_bounded_ascii_decimal(value)
        and int(value) == body_length
    )
...
def _fetch_sources(source_urls: list[str]) -> tuple[list[dict], list[str]]:
    ...
    for index, url in enumerate(source_urls):
        ...
        if status in (408, 425, 429) or status >= 500:
            raise gl.vm.UserError(
                f"{ERROR_TRANSIENT} Source {index} is temporarily unavailable"
            )

        body = response.body or b""
        content_range = _header_text(response.headers, "content-range")
        range_response = status == 206 or len(content_range) > 0
        if range_response and not _content_range_proves_complete(
            content_range,
            len(body),
        ):
            fetched.append(
        ...
        if status in (200, 206) and not _content_length_matches(
            content_length,
            len(body),
        ):
            fetched.append(
        ...
        if (
            ...
            or len(body) == 0
            or not _media_type_is_textual(media_type)
        ):
            fetched.append(
                _source_fetch_result(
                    index,
        ...
        if (
            len(normalized) > MAX_EVIDENCE_CHARS
            or normalized_utf8_bytes > MAX_EVIDENCE_UTF8_BYTES
        ):
            fetched.append(
                _source_fetch_result(
                    index,
...
def _canonical_relation(value) -> str:
    ...
    aliases = {
        "SUPPORTED": RELATION_SUPPORTS,
        "SUPPORT": RELATION_SUPPORTS,
        "PARTIALLY_SUPPORTED": RELATION_PARTIAL,
        "PARTIALLY_SUPPORTS": RELATION_PARTIAL,
        "CONTRADICTED": RELATION_CONTRADICTS,
        "CONTRADICTION": RELATION_CONTRADICTS,
        "IRRELEVANT": RELATION_NO_RELEVANT,
        "INSUFFICIENT_EVIDENCE": RELATION_NO_RELEVANT,
    }
...
def _parse_source_classifications(
    analysis,
    fetched_sources: list[dict],
    contents: list[str],
) -> list[dict]:
    ...
    for raw_result in raw_results:
        if (
            ...
            or len(raw_result) != 4
            or "counter_excerpt" not in raw_result
            or "evidence_excerpt" not in raw_result
            or "index" not in raw_result
            or "relation" not in raw_result
        ):
        ...
        evidence_excerpt = _validated_excerpt(
            raw_result.get("evidence_excerpt", ""),
            "evidence_excerpt",
        )
        counter_excerpt = _validated_excerpt(
            raw_result.get("counter_excerpt", ""),
            "counter_excerpt",
        )
        ...
        if not valid_shape:
            raise gl.vm.UserError(
                f"{ERROR_LLM} Excerpts do not match the source relation"
            )

        content = contents[index]
        if len(evidence_excerpt) > 0 and evidence_excerpt not in content:
            raise gl.vm.UserError(
                f"{ERROR_LLM} evidence_excerpt was not copied from the source"
            )
        if len(counter_excerpt) > 0 and counter_excerpt not in content:
            raise gl.vm.UserError(
                f"{ERROR_LLM} counter_excerpt was not copied from the source"
            )

        parsed_by_index[index] = {
            "index": index,
            "fetch_status": FETCH_AVAILABLE,
            "relation": relation,
            "evidence_excerpt": evidence_excerpt,
            "counter_excerpt": counter_excerpt,
            "content_truncated": fetched_sources[index]["content_truncated"],
        }

    if len(parsed_by_index) != len(available_indices):
        raise gl.vm.UserError(
            f"{ERROR_LLM} Grounding response did not classify every available source"
        )
    ...
    for source in fetched_sources:
        ...
        if source["fetch_status"] == FETCH_UNAVAILABLE:
            results.append(
                {
                    "index": index,
                    "fetch_status": FETCH_UNAVAILABLE,
                    "relation": RELATION_NOT_EVALUATED,
                    "evidence_excerpt": "",
                    "counter_excerpt": "",
...
def _derive_verdict(source_results: list[dict]) -> tuple[str, str]:
    ...
    if mixed_count > 0 or (
        contradict_count > 0 and (support_count > 0 or partial_count > 0)
    ):
        ...
        source_results.append(
            {
                ...
                "evidence_excerpt": "",
                "counter_excerpt": "",
                "content_truncated": fetch_status == FETCH_TRUNCATED,
            }
        )
...
def _prepare_grounding(
    ...
    if len(prompt.encode("utf-8")) > evaluation_prompt_limit:
        limited_sources, limited_contents = _prompt_limit_truncation(
            fetched_sources,
            contents,
        )
        return limited_sources, limited_contents, ""
...
def _evaluate_grounding(
    ...
    fetched_sources, contents, prompt = _prepare_grounding(
        claim,
        source_urls,
    )
    if len(prompt) == 0:
        ...
        source_results = _parse_source_classifications(
            analysis,
            fetched_sources,
            contents,
        )
...
def _grounding_audit_prompt(
    claim: str,
    source_urls: list[str],
    leader_result: dict,
    independently_fetched_sources: list[dict],
) -> str:
    ...
    for fetched_source in independently_fetched_sources:
        ...
        audit_sources.append(
            {
                "content": fetched_source["content"],
                "index": index,
                "proposed_counter_excerpt": leader_source["counter_excerpt"],
                "proposed_evidence_excerpt": leader_source["evidence_excerpt"],
                "proposed_relation": leader_source["relation"],
                "url": source_urls[index],
            }
        )
...
def _audit_accepts_all(analysis, available_indices: list[int]) -> bool:
    ...
    for raw_source in raw_sources:
        if (
            ...
            or len(raw_source) != 2
            or "accept" not in raw_source
            or "index" not in raw_source
        ):
        ...
        if (
            ...
            or index not in available_indices
    ...
    if len(decisions) != len(available_indices):
        raise gl.vm.UserError(
            f"{ERROR_LLM} Audit did not cover every available source"
        )
...
def _validate_result(
    result,
    source_count: int,
    independently_fetched_contents: list[str],
) -> None:
    if not isinstance(result, dict):
        raise gl.vm.UserError(f"{ERROR_LLM} Grounding result must be an object")
    if (
        len(result) != 3
        or "reason_code" not in result
        or "sources" not in result
        or "verdict" not in result
    ):
    ...
    for source in sources:
        if (
            ...
            and source.get("fetch_status") == FETCH_TRUNCATED
        ):
    ...
    for expected_index, source in enumerate(sources):
        ...
        if (
            len(source) != 6
            or "content_truncated" not in source
            or "counter_excerpt" not in source
            or "evidence_excerpt" not in source
            or "fetch_status" not in source
            or "index" not in source
            or "relation" not in source
        ):
        ...
        if (
            len(evidence_excerpt) > MAX_EXCERPT_CHARS
            or len(counter_excerpt) > MAX_EXCERPT_CHARS
            or (0 < len(evidence_excerpt) < MIN_EXCERPT_CHARS)
            or (0 < len(counter_excerpt) < MIN_EXCERPT_CHARS)
        ):
        ...
        if fetch_status == FETCH_UNAVAILABLE:
            if (
                relation != RELATION_NOT_EVALUATED
                or len(evidence_excerpt) > 0
                or len(counter_excerpt) > 0
                or content_truncated
                or len(independent_content) > 0
            ):
            ...
            if relation == RELATION_NOT_EVALUATED:
                if (
                    not batch_has_truncated
                    or len(evidence_excerpt) > 0
                    or len(counter_excerpt) > 0
                ):
                    raise gl.vm.UserError(
                        f"{ERROR_LLM} Invalid skipped source result"
                    )
                continue
            if batch_has_truncated or relation not in _ACTIVE_RELATIONS:
                raise gl.vm.UserError(f"{ERROR_LLM} Invalid available source result")
            if (
                evidence_excerpt != _normalize_excerpt(evidence_excerpt)
                or counter_excerpt != _normalize_excerpt(counter_excerpt)
            ):
            ...
            if relation == RELATION_SUPPORTS:
                ...
                valid_shape = (
                    len(evidence_excerpt) > 0 and len(counter_excerpt) == 0
                )
            elif relation == RELATION_CONTRADICTS:
                valid_shape = len(evidence_excerpt) == 0 and len(counter_excerpt) > 0
            elif relation == RELATION_MIXED:
            ...
            if len(evidence_excerpt) > 0 and evidence_excerpt not in independent_content:
                raise gl.vm.UserError(
                    f"{ERROR_LLM} Leader evidence was not found by the validator"
                )
            if len(counter_excerpt) > 0 and counter_excerpt not in independent_content:
                raise gl.vm.UserError(
                    f"{ERROR_LLM} Leader counter-evidence was not found by the validator"
                )
    ...
    if verdict != derived_verdict or reason_code != derived_reason:
        raise gl.vm.UserError(
            f"{ERROR_LLM} Final verdict was not derived from source results"
        )


def _consensus_fields_match(leader_result: dict, validator_result: dict) -> bool:
    if (
        leader_result["verdict"] != validator_result["verdict"]
        or leader_result["reason_code"] != validator_result["reason_code"]
    ):
        return False
    ...
    for index in range(len(leader_sources)):
        ...
        if (
            leader_source["fetch_status"] != validator_source["fetch_status"]
            or leader_source["relation"] != validator_source["relation"]
            or leader_source["content_truncated"]
            != validator_source["content_truncated"]
        ):
            return False
        if leader_source["relation"] == RELATION_NOT_EVALUATED and (
            len(leader_source["evidence_excerpt"]) > 0
            or len(leader_source["counter_excerpt"]) > 0
        ):
            return False
    return True
...
def _fetch_fields_match(leader_result: dict, fetched_sources: list[dict]) -> bool:
    ...
    for index in range(len(leader_sources)):
        ...
        if (
            leader_source["fetch_status"] != fetched_source["fetch_status"]
            or leader_source["content_truncated"]
            != fetched_source["content_truncated"]
        ):
            return False
    return True
...
class AgentOutputGroundingVerifier(gl.Contract):
    ...
    def verify_claim(self, claim: str, source_urls_json: str) -> u256:
        if len(claim) > MAX_CLAIM_CHARS:
            raise gl.vm.UserError(
                f"{ERROR_EXPECTED} Claim length is outside the allowed range"
            )
        normalized_claim = claim.strip()
        if (
            len(normalized_claim) < MIN_CLAIM_CHARS
            or len(normalized_claim) > MAX_CLAIM_CHARS
        ):
            raise gl.vm.UserError(
                f"{ERROR_EXPECTED} Claim length is outside the allowed range"
            )
        for character in normalized_claim:
            ...
            if (
                ...
                raise gl.vm.UserError(
                    f"{ERROR_EXPECTED} Claim cannot contain control characters"
                )

        source_urls, canonical_sources_json = _canonical_sources(
            source_urls_json,
            self.allowed_domains_json,
        )

        def leader_fn() -> dict:
            result, leader_contents = _evaluate_grounding(
                normalized_claim,
                source_urls,
            )
            _validate_result(
        ...
        def validator_fn(leaders_res: gl.vm.Result) -> bool:
            ...
            try:
                ...
                if len(evaluation_prompt) == 0:
                    validator_result = _result_from_sources(
                        _not_evaluated_results(validator_sources)
                    )
                    _validate_result(
                        validator_result,
                        len(source_urls),
                    ...
                    return _consensus_fields_match(
                        leaders_res.calldata,
                        validator_result,
                    )
                ...
                for source in validator_sources:
                    if source["fetch_status"] == FETCH_AVAILABLE:
                        available_indices.append(source["index"])
                audit_prompt = _grounding_audit_prompt(
                    normalized_claim,
                    source_urls,
                    leaders_res.calldata,
                    validator_sources,
        ...
        for source in result["sources"]:
            sanitized_source_results.append(
                {
                    "index": source["index"],
                    "fetch_status": source["fetch_status"],
                    "relation": source["relation"],
                    "evidence_excerpt": source["evidence_excerpt"],
                    "counter_excerpt": source["counter_excerpt"],
        ...
        self.verifications[verification_id] = Verification(
            ...
            sources_json=canonical_sources_json,
            request_digest=request_digest,
            source_count=u8(len(source_urls)),
            verdict=result["verdict"],
            reason_code=result["reason_code"],
            source_results_json=source_results_json,
            policy_version=POLICY_VERSION,
            scope=VERIFICATION_SCOPE,
            transaction_timestamp=_now_iso(),
        )
    ...
    def get_verification(self, verification_id: u256) -> dict:
        ...
        return {
            ...
            "source_results_json": record.source_results_json,
    ...
    def get_policy(self) -> dict:
        return {
            ...
            "max_evidence_utf8_bytes": MAX_EVIDENCE_UTF8_BYTES,
            "max_prompt_bytes": MAX_PROMPT_BYTES,
            "audit_prompt_headroom_bytes": AUDIT_PROMPT_HEADROOM_BYTES,
            "min_excerpt_chars": MIN_EXCERPT_CHARS,
            "max_excerpt_chars": MAX_EXCERPT_CHARS,
            "allowlist_required": True,
            "allowlist_scope": "INITIAL_REQUEST_HOSTNAME_ONLY",
            "redirect_destination_observable": False,
            "redirect_destination_enforced": False,
            "source_url_queries_allowed": False,
            "content_length_mismatch_status": FETCH_TRUNCATED,
            "claim_atomicity_enforced": False,
            "caller_must_supply_atomic_claim": True,
            "verdicts_json": _canonical_json(list(_VERDICTS)),
            "relations_json": _canonical_json(list(_STORED_RELATIONS)),
            "classifier_relations_json": _canonical_json(
                list(_ACTIVE_RELATIONS)
            ),
        }
